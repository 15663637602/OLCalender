...
function article_xiajia(obj,id){
	...
	$(obj).parents("tr").find(".article-status").html('<span class="label">未发布</span>');
	$(obj).remove();
}
/*资讯-发布*/
...
function picture_xiajia(obj,id){
	...
	$(obj).parents("tr").find(".picture-status").html('<span class="label">未发布</span>');
	$(obj).remove();
}
/*图片库-发布*/
...
/*管理员-删除*/
function admin_del(obj,id,String){
	alert("asd");
	layer.confirm('确认要删除吗？',function(index){
		var name = String;
		var req = false;
